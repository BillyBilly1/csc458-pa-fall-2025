...
static void send_icmp_echo_reply(struct sr_instance *sr, uint8_t *packet, unsigned int len, struct sr_if *iface) {
  ...
  sr_icmp_hdr_t *icmp_r = (sr_icmp_hdr_t *)(reply + sizeof(sr_ethernet_hdr_t) + ip_r->ip_hl * 4);
  ...
  ip_r->ip_dst = temp;
  ip_r->ip_sum = 0;
  ip_r->ip_sum = cksum(ip_r, ip_r->ip_hl * 4);

  icmp_r->icmp_type = 0;
  icmp_r->icmp_code = 0;
  icmp_r->icmp_sum = 0;
  icmp_r->icmp_sum = cksum(icmp_r, len - sizeof(sr_ethernet_hdr_t) - ip_r->ip_hl * 4);
...
static void handle_ip(struct sr_instance *sr, uint8_t *packet, unsigned int len, char *interface) {
  ...
  if (ip_hdr_len < 20 || len < sizeof(sr_ethernet_hdr_t) + ip_hdr_len) {
    return;
  }
  ...
  if (to_me) {
    if (ip->ip_p == ip_protocol_icmp) {
      ...
      send_icmp_t3(sr, packet, interface, 3, 3);
    }
    return;
  }
  ...
  struct sr_rt *best = longest_prefix_match(sr, ip->ip_dst);
  if (!best) {
    send_icmp_t3(sr, packet, interface, 3, 0);
    return;
  }
